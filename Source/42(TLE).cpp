...
vector<int> cut_head_and_tail(vector<int> raw) {
    ...
    while (head_0 < raw.size() && raw[head_0] == 0) head_0++;
    while (tail_0 < raw.size() && raw[raw.size() - 1 - tail_0] == 0) tail_0++;

    if (head_0 + tail_0 >= raw.size()) return {};
...
vector<int> cut_min(vector<int> raw) {
    ...
    for (int i = 0; i < raw.size(); i++)
        if (raw[i] < min_h) min_h = raw[i];
    for (int i = 0; i < raw.size(); i++) raw[i] -= min_h;
    return raw;
}

bool check(vector<int> arr) {
    if (!arr.empty() && (arr[0] == 0 || arr.back() == 0)) return false;
    return true;
}
